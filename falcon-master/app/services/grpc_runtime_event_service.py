...
from app.models import TaskMetricSecond, TaskRun, Tasks
from falcon_shared.runtime_enums import TaskRunStatusEnum, TaskStatusEnum
...
class GrpcRuntimeEventService:
    async def handle_event(
        ...
        *,
        ...
        try:
            ...
            merged_summary = self._merge_summary_with_runtime_metadata(
                summary=summary,
    ...
    def _merge_summary_with_runtime_metadata(
        self,
        *,
        ...
        normalized_worker_task_id = str(worker_task_id or "").strip()

        if normalized_worker_id:
            merged_summary["worker_id"] = normalized_worker_id
        elif existing_summary.get("worker_id"):
            merged_summary["worker_id"] = existing_summary["worker_id"]

        if normalized_worker_task_id:
            merged_summary["worker_task_id"] = normalized_worker_task_id
        elif existing_summary.get("worker_task_id"):
            merged_summary["worker_task_id"] = existing_summary["worker_task_id"]

        if existing_summary.get("worker_addr") and not merged_summary.get("worker_addr"):
            merged_summary["worker_addr"] = existing_summary["worker_addr"]
