...
class GrpcServer(worker_runtime_pb2_grpc.ControlPlaneRuntimeServicer):
...
class ControlPlaneGrpcServer:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._server.start, name="grpc_master-control-plane", daemon=True)
        self._thread.start()
...
grpc_server = ControlPlaneGrpcServer()
