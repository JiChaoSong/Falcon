...
const projectList = ref<ProjectInfoList[]>()
...
const showEditModal = (projectId: string) => {
  ...
  state.formData = {
    ...
    tags: [...project.tags],
    targetTps: project.targets.tps,
    targetRt: project.targets.responseTime
  }
  state.modalVisible = true
}
...
const handleModalOk = () => {
  ...
  if (state.isEditing && state.currentEditId) {
    ...
    // 新增项目
    const newId = `PROJECT-${String(state.projects.length + 1).padStart(3, '0')}`
    ...
    lastTaskDate.setDate(lastTaskDate.getDate() - Math.floor(Math.random() * 10))
    const lastTask = lastTaskDate.toLocaleString('zh-CN')
      ...
      content: h('div', {}, [
        h(Descriptions, { column: 2, bordered: true }, {
          default: () => [
            ...
            h(DescriptionsItem, { label: '负责人' }, () => project.owner),
            h(DescriptionsItem, { label: '状态' }, () =>
                h(Tag, { color: statusMap[project.status].color }, statusMap[project.status].text)
            ),
            h(DescriptionsItem, { label: '优先级' }, () => priorityMap[project.priority]),
            h(DescriptionsItem, { label: '标签' }, () =>
                project.tags.map(tag =>
                    h(Tag, { color: tagColors[tag], key: tag }, tagNames[tag])
                )
            ),
            h(DescriptionsItem, { label: '场景数' }, () => project.stats.scenarios),
            h(DescriptionsItem, { label: '任务数' }, () => project.stats.tasks),
            h(DescriptionsItem, { label: '成功率' }, () => `${project.stats.successRate}%`),
            h(DescriptionsItem, { label: '平均响应时间' }, () => `${project.stats.avgResponseTime}ms`),
            h(DescriptionsItem, { label: '创建时间' }, () => project.createdAt),
            h(DescriptionsItem, { label: '最后更新' }, () => project.updatedAt),
...
const handleGetProjectList = async (data:QueryProjectList) => {
  try {
    state.formData.page = state.currentPage
    const response = await ProjectApi.getProjectList(data);
...
const columns = [
  ...
  },
  {
    ...
    customRender: ({ text }: { text: string }) => {
      return h(Space, {}, [
        h(Avatar, { size: 'small', style: { backgroundColor: '#1890ff' } }, text.charAt(0)),
        h('span', {}, text)
      ])
    }
  },
  {
    ...
    width: 120,
    customRender: ({ text }: { text: string }) => {
      return h(Tag, { color: statusMap[text].color }, statusMap[text].text)
    }
  },
  {
    title: '标签',
...
onMounted(() => {
  handleGetProjectList(state.formData)
})
...
<template>
  <div class="app-container">
    ...
    <div v-if="state.viewMode === 'card'">
      <!-- 卡片视图 -->
      <Row :gutter="[16, 16]" v-if="projectList.length > 0">
        <Col :span="8" v-for="project in projectList" :key="project.id">
          <ACard class="project-card" hoverable>
            ...
            <div class="project-meta">
              ...
              <div class="meta-item">
                ...
                <div class="meta-value">{{ project.stats.scenarios }}</div>
              </div>

              <div class="meta-item">
                <span class="meta-label">任务数</span>
                <div class="meta-value">{{ project.stats.tasks }}</div>
              </div>
            </div>
            ...
            <div class="project-performance">
              ...
              <div class="performance-item">
                <div class="performance-value">{{ project.stats.avgResponseTime }}ms</div>
                <div class="performance-label">平均响应</div>
              </div>
              <div class="performance-item">
                <div class="performance-value" style="color: #1890ff;">
                  {{ project.stats.lastTask.split(' ')[0] }}
                </div>
    ...
    <!-- 分页 -->
    <div class="pagination-container" v-if="state.filteredProjects.length > 0">
      <Pagination
          v-model:current="state.currentPage"
          v-model:pageSize="state.pageSize"
          :total="state.filteredProjects.length"
          show-quick-jumper
          show-size-changer
